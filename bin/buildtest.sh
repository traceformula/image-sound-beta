...
javac -g -cp src -d build/packages -Xlint:unchecked src/ts/Main.java
pwd
# now compile the runtime support
javac -g -cp src -d build/packages -Xlint:unchecked src/ts/support/*.java

# now run javadoc
#javadoc -classpath build/packages -d build/html -use -package -quiet -doctitle "ts" src/ts/*.java src/ts/parse/*.java src/ts/support/*.java src/ts/tree/*.java src/ts/tree/visit/*.java
